...
bool Knight::Test(ostream& os)
{
	...
	for (vector<Location>::const_iterator validIter = validMoves.begin(); validIter != validMoves.end(); validIter++)
	{
		...
		{
			os << "ERROR: knight did not give move " << validIter->row() << ", " << validIter->column() << " when it was expected to!" << endl;
			return false;
		}
	}
	...
	if (givenMoves.size() > 0)
	{
		...
		{
			os << "ERROR: knight gave extra move " << extraIter->row() << ", " << extraIter->column() << endl;
		}
		return false;
	}
